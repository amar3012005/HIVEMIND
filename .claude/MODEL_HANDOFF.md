...
## Current production context (2026-07-09)

- Treat `.claude/MEMORY.md` as the current production source of truth. Legacy references to
  `myserver`, Coolify, `/opt/HIVEMIND`, or `hivemind.davinciai.eu` are historical and must not
  drive deployments.
- Production runs Docker Compose on `root@singulancelabs.com`. Build from the clean
  `/root/hivemind-next` clone; execute production Compose only from `/root/hivemind/infra`
  with `--env-file /root/hivemind/.env`. The live checkout is dirty by design.
- The vNext frontend is one shared frontend on `next.singulancelabs.com`, connected to existing
  production control/core hosts. Do not split B2B and B2C frontends without a new explicit
  decision; the isolated B2B/B2C backend canaries are not customer traffic.
- Platform admin lives at `/hivemind/platform-admin`, uses a short-lived signed HttpOnly cookie,
  and has authenticated two-second control-plane log polling. Never disclose the environment
  secret or reintroduce public raw log endpoints.
