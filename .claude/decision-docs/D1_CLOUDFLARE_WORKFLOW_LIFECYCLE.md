...
- lists, emphasis, links, quotes, code, horizontal rules, and pipe tables are rendered;
- table alignment is preserved, cells wrap long values, and rows avoid print-page splits;
- UTF-8 source characters remain intact across Latin, CJK, Indic, Arabic, symbols, currency, and emoji;
- unsafe HTML is escaped;
- line wrapping and pagination may change;
- words, claims, citations, and conclusions are not rewritten.

The hero includes the exact deterministic Humation portraits for the persisted
company team. Email uses the same public avatar endpoint as Day 0; PDF embeds
...
headline, destination URL, sealed output, and participating characters. Day 1
is a thin preset over those contracts. No future episode should fork the
Markdown/table/Unicode/character renderer or introduce a second PDF service.
