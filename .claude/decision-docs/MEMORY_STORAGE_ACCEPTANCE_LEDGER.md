...
| Field | Evidence |
|---|---|
| Canonical merged SHA | `PASS` — storage runtime included in `7b4034368fc981fcac11e3c8b5f0b31d269fb3a4`; isolated restore automation through `d7688e0738c049f9e4816a268954373b880ea4a9`; portable/off-host AMR recovery through `562c4da8bc86941289985c699c7e17218de284c3`; encrypted BYOD replay through `dc51d2a3e48626238605126df7028c4192d70d9c`; managed tenant admission through `b3c2382e24997e17b5f38a676c4fe969fdbe18bc`; managed PITR through `71363e50d58b9927e49542b40dd616b0e63f4bc6`; signed BYOD upgrade/rollback drill through `e55880cc4ebacbe369e4a04690984046dba47899`; Unicode `.amr` lexical through `5975a2791784e29e32e03762c2c2e68b91681c96`; managed Unicode lexical through `04a434059843e5e654282f8700668c8b09375c65`; cold cross-mode evidence recall through `a9bcff3d4be374869137db2e4e1c11b7d58bb1e8` |
| Immutable images | `PASS` — Core `sha-a9bcff3d`; PostgreSQL `sha-71363e50`; Memory Box agent `sha-7b403436` |
| Migration IDs | `PASS` — production was baselined at canonical SHA `0092afa99445109e940ad1aa4ac44c43ee2ab845` on 2026-08-17 with 160/160 repository migration IDs. An isolated production-clone drill first proved the official Prisma resolve path produced 160 records and changed no application DDL (`883dbec7…eff2` before/after). Production then used one atomic metadata-only transaction under the release lock; `prisma migrate status` reports up to date and the application-schema hash excluding only `_prisma_migrations` stayed `4ebd1ff3…1de0`. Pre-change dump `/var/backups/hivemind/migration-baseline-20260817T135007Z.dump` is 137,684,521 bytes, SHA-256 `7fecb6a4…bb8c`, mode `0600`, with an independently verified macOS copy. Receipt: `/var/lib/hivemind/migration-baseline/20260817T135351Z.json` |
| Prisma model/schema alignment | `FAIL` — the migration ledger is now safe and current, but a read-only Prisma schema diff still reports material intentional/manual drift (including operational tables not represented in `schema.prisma` and model/index differences). Reconcile this in reviewed additive batches; do not auto-apply the destructive diff |
...
| Evidence-only public lifecycle | `PASS` — immutable Core `sha-a9bcff3d` stored one embedded shard segment, cold public quick recall returned it before any direct evidence warm-up, central document/segment/memory counts stayed `0/0/0`, and deletion removed it | `PASS` — the same canary stored one central segment and zero memories, cold public quick recall returned it, central counts were `1/1/0` before cleanup, and deletion removed it | `PASS` — the same canary stored one remote segment, direct remote lexical/hydration returned `4/4`, cold public quick recall returned 2 grounded rows, central counts stayed `0/0/0`, and deletion removed it. The temporary API keys/documents were removed in every mode; reusable canary: `core/scripts/byod-evidence-lifecycle-canary.mjs`; release manifest: `/root/releases/manifests/a9bcff3d/20260817T141647Z/RELEASE_MANIFEST.json` |
