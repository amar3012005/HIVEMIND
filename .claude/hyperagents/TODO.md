...
- [~] **P6 — Outreach Contract v1 (TARA autonomy)** — LAST, GATED on P0. Report emits contract; TARA picks skill + sets goal; outcome writes back verified learning.
  - [x] **Contract envelope SHIPPED 2026-07-25** (`eea260835`) — `core/src/outreach/contract.js`,
    provider-neutral v1 + `resolveDelivery()`. Any provider (deepgram/grok/future/browser) can
    fulfil it; `provider.preferred` is a hint only; vN/vN-1 tolerant.
  - [x] **Browser-call fallback SHIPPED** — provider `/capabilities` probe; no telephony →
    target parked `state:'browser'` with the contract instead of a hard `failed`.
  - [ ] **FE affordance** — render `state:'browser'` targets in CampaignPanel/LeadsView with a
    "Call from browser" button that opens the TARA voice widget pre-loaded from
    `payload.contract` (goal/opener/strategy → directive, persona.language/voice_id).
  - [ ] Outcome write-back (`{outcome, learnings}` → memory) still pending; still gated on P0.

### Fixed 2026-07-25 (outside the phase plan)
- [x] **Rooms now use Google Maps.** Not a wiring gap — `_wants_discovery()` (engine.py ~586)
  blocked it: no `generate` verb ("generate leads" failed) and generic-nouns-only, so concrete
  business types ("dental clinics in Munich") never triggered Places. Broadened verbs/nouns +
  geo-anchored fallback. 12/12 discovery pass, 6/6 drafting still blocked.
