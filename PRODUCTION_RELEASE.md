...
## prod-20260907-39a8c5b3 — account-scoped governed HyperAgents Room canary
- **Date:** 2026-09-07
- **Parent:** `singulance-main` at `39a8c5b313049e560e3dde49e2ed0d642b9bdb7a`; frontend unchanged at gitlink `f5a987b12e5959f886d96b6588b0f42b8f6a23d8`.
- **Scope:** only Control Plane was promoted through the canonical service-scoped release. The existing `hm-employees` service remained on accepted image `hivemind/employees:sha-549fc37a`; the governed HyperAgents implementation runs in isolated healthy container `hm-employees-hyper-canary` on image `hivemind/employees-hyper-canary:sha-39a8c5b3`. No migration, frontend deployment, Core replacement, or unrelated service replacement occurred.
- **Flag:** Cloudflare Flagship boolean `hyperagents_governed_room_v1` defaults off and serves on only when environment, normalized authenticated email, organization ID, and user ID all match the canary account. Production evaluation returned true for the exact canary tuple and false for both a mismatched user and mismatched email. Evaluation fails closed.
- **Durability:** production Cloudflare Workers were deployed for canonical projection and the evidence, output, and prospect lifecycle boundaries. Only opaque tenant/job identifiers cross those workflow boundaries; approval and evidence gates remain authoritative.
- **Tests:** canonical focused Node lifecycle tests 12/12; Python HyperAgents tests 56/56; canonical projection Worker 16/16 plus typecheck and production dry run; evidence/output/prospect Worker typechecks and production dry runs; packaged runtime import passed.
- **Acceptance:** authenticated room turn `4aa7c4a3-0f87-4767-a5ef-5b63be7c83c2` completed through `hm-employees-hyper-canary`; its persisted first event was `governed_room_canary=true`. Candidate logs showed organization-profile loading and Cloudflare AI Gateway/OpenRouter model calls, with no external tools or artifacts requested. Public homepage and API health returned 200; Control, existing Employees, and candidate Employees were healthy.
- **Manifest:** `/root/releases/manifests/39a8c5b3/20260907T091429Z/RELEASE_MANIFEST.json`.
- **Rollback:** Control Plane can be restored to immutable image `hivemind/control-plane:sha-030879af`. Disabling `hyperagents_governed_room_v1` immediately routes all new turns to the unchanged existing Employees path; the isolated candidate can then be stopped independently.

## prod-20260907-f8fc0d6f — immediate admission and evidence-safe GDPR audit canary
- **Date:** 2026-09-07
- **Parent:** `singulance-main` at `f8fc0d6f6952432e60a8d0b6d0731aad7d52e906`; frontend was not deployed.
- **Scope:** Control Plane alone was promoted through the canonical service-scoped release at `eab8fff43f6e8eb691b54ccdd2941c653857f7b3`. The governed HyperAgents candidate alone was then rebuilt from `f8fc0d6f6952432e60a8d0b6d0731aad7d52e906`. Existing `hm-employees` stayed healthy and unchanged on `hivemind/employees:sha-fe259407`; no Core, frontend, migration, or unrelated service was deployed.
- **Fast admission:** Control resolves the authenticated account email from the canonical user row, evaluates the canary and planner flags concurrently, and persists both the canary decision and a deterministic acknowledgement before returning HTTP 202. Authenticated production turn `a0e48b7b-3fa6-4cf9-9278-03d831d7aaec` was admitted in 269 ms with `governed_room_canary=true` and `selected_runtime=governed_candidate`.
- **Governed audit:** the legal/finance research floor loaded the organization profile, forced the existing Ravi Patel, Marta Silva, and Elena Kovacs room employees into debate, searched the connected sent Gmail scope, ran Composio web search against EDPB and European Commission domains, and split Playwright extraction into one durable job per public origin. The accepted receipt set contained 24 rendered Singulance pages and five regulator sources. Instagram rendering failed explicitly and remained an unresolved gap.
- **Fail-closed output:** the verifier rejected unsupported exclusivity, legal-article, deadline, and numeric-control claims. The rejected draft remains only in the auditable work-room checkpoint; the final report withholds it, exposes the evidence links and exact missing channels, and seals `blocked` rather than presenting unsupported legal advice as fact.
- **Tests:** focused Employees suites passed 69/69 after the final changes; the multi-origin crawl regression proves each origin receives its own job and receipt. Python compile and `git diff --check` passed. Fresh production fatal scans for Control and the governed candidate returned zero.
- **Flag proof:** production evaluation returned true for the exact `amarsai2005@gmail.com` organization/user tuple and false for a mismatched identity. Failure remains closed to the unchanged stable Employees runtime.
- **Images and manifest:** Control `hivemind/control-plane:sha-eab8fff4` is healthy with manifest `/root/releases/manifests/eab8fff4/20260907T095802Z/RELEASE_MANIFEST.json`; candidate `hivemind/employees-hyper-canary:sha-f8fc0d6f` is healthy.
- **Rollback:** disable `hyperagents_governed_room_v1` for immediate stable routing. The prior candidate is retained as stopped container `hm-employees-hyper-canary-rollback-5e26431c`; Control retains its canonical rollback image through the service-scoped release manifest.
