...
hm_allowed_env_key() {
  case "$1" in
    ...
    TS_AUTHKEY|TS_HOSTNAME|TAILSCALE_IMAGE|BYOD_COMPOSE_PROJECT_NAME|\
    HIVEMIND_HARNESS_IMAGE|HIVE_HARNESS_TICKET_SECRET|HIVE_HARNESS_RUNNER_SERVICE_SECRET|\
    HIVEMIND_HARNESS_PARENT_ORIGINS|HIVEMIND_HARNESS_TRUSTED_HOSTS|HARNESS_BIND|HARNESS_PORT|\
    HARNESS_PUBLIC_URL|REDIS_IMAGE|HIVEMIND_USER_ID|HIVEMIND_INVITED_USER_IDS) return 0 ;;
    *) return 1 ;;
  esac
}
