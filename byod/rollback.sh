...
RECEIPT="$HERE/.releases/CURRENT_RELEASE.json"
...
OVERRIDE="$HERE/.releases/manual-rollback-$(date -u +%Y%m%dT%H%M%SZ).yml"
...
for _ in $(seq 1 45); do
  STATE="$(docker inspect hm-byod-agent --format '{{.State.Status}}' 2>/dev/null || true)"
  [[ "$STATE" == running ]] && break
  sleep 2
done
[[ "$STATE" == running ]]
