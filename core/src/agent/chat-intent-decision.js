          ...
          save: {
            ...
            properties: {
              ...
              scope: {
                ...
                description: 'Destination explicitly requested by the user. Set personal, organization, team, or project only when the user states that destination or selects it from a prior scope choice. Leave absent when the destination was not stated; the server will ask before saving.',
              },
  ...
  const normalized = {
    ...
    save: raw.save && boundedText(raw.save.content)
      ? {
          ...
          scope: SCOPES.has(raw.save.scope) ? raw.save.scope : null,
...
For implicit durable facts or attributable user assertions, use save only at confidence >= 0.80. For explicit save continuations, resolve the immediately preceding user statement from history; if it is only an assertion, preserve it as admission_class=user_assertion. Memory destination is always user-controlled: set save.scope or save.project_id/project_hint only when the user explicitly named a destination in this turn, or selected one in the immediately preceding scope-choice continuation. If no destination was stated, leave all three absent. The server will pause and ask where to save; never silently infer personal, organization, team, or project scope from the content, profile, current page, or project catalog. For save/update, choose project_id only from the authorized project catalog below; if no project clearly fits, leave project_id null. Never invent a project id.
...
Use scope_filter=personal for questions specifically about the user. Saving a NEW fact is additive and needs no permission, but it does require an explicit destination. Never ask whether to save a fact the user has already asked or stated; when scope is absent, ask only where it belongs. Only a destructive write (delete) needs approval, and that is enforced in code by a server-issued one-time token, not by asking here. Never broaden organization or project scope.
