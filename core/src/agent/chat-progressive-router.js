...
const NATIVE_TOOL_CATALOG = [
  ...
  'hivemind_find_entities — a read-only, tenant-authorized entity directory lookup for a partial or ambiguous name (for example "Uwe") before a targeted recall; not a general fact-answering tool.',
...
For a grounded read, use hivemind_context and follow the native_tool catalog attached to that field; its descriptions are authoritative. Set operation to entity_discovery for hivemind_find_entities when the user asks whether a partial/ambiguous person, company, project, or topic exists or asks to list matching identities (for example "find Uwe", "which Amars exist?"). Do not use it for a general question about an already exact subject; use recall then. Set operation to recall/source_read/temporal_range for hivemind_recall, temporal for hivemind_at, diff for hivemind_diff, timeline for hivemind_timeline, aggregate for hivemind_aggregate_entities, and relation_between for hivemind_relation_between. For a named file, preserve its closest recognizable title in source_title and use operation=source_read. For the latest/recent/first uploaded record, use hivemind_recall with temporal_semantics latest/earliest plus source_kind (image when it is an image); this is ordered retrieval, never a snapshot time-travel call. Set temporal_axis=valid_time or known_time only for hivemind_at and none otherwise.
...
const CONTEXT_OP = {
  recall: 'recall', entity_discovery: 'entity_discovery', source_read: 'source_read', aggregate: 'aggregate',
  ...
  switch (tool) {
    case 'hivemind_context': {
      ...
      const nativeOp = ({
        ...
        hivemind_find_entities: 'entity_discovery',
