...
export const HIGH_TOOLS = [
  { type: 'function', function: { name: 'hivemind_context', strict: true,
    ...
    parameters: object({
      ...
      answer_scope: { type: 'string', enum: ['bounded', 'broad', 'exhaustive'], description: 'Semantic coverage contract, independent of wording or language. bounded = one answerable point; broad = several relevant facets or useful members; exhaustive = the user needs the complete retained set of relevant claims/items, including material continued through conversation context. This controls the minimum evidence window, not writing style.' },
...
Select answer_scope before response_depth from semantic breadth, in any language: bounded for a fact or ordinary question; broad for a multi-aspect explanation, overview, comparison, or informative inventory; exhaustive for a complete cross-source treatment. Its minimum response_depth is bounded => standard, broad => detailed, exhaustive => comprehensive. Standard exposes the unified top 5; detailed and comprehensive expose the unified top 15. Retrieval itself retains the top 15 in one pass. There is no later 5-to-10-to-15 hop.
...
export function adaptToDecision(tool, args, message, language, { useTools = false } = {}) {
  ...
  const answerScope = ['bounded', 'broad', 'exhaustive'].includes(args?.answer_scope)
    ? args.answer_scope
    : 'bounded';
