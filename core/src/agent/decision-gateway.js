...
export const CAPABILITY_OPTIONS = Object.freeze([
  { id: 'direct_answer', criteria: 'Answer only from the current request, compact authenticated context, and recent turns when the answer does not require a profile lookup, stored memory, fresh retrieval, external application, web verification, or a state change. Do not select for identity or profile questions about the authenticated user or organization. Never use for a request to save, update, search, send, or create.' },
  { id: 'hivemind_context', criteria: 'Use the authenticated compact profile/context for questions about the current user or organization: who am I, what do you know about me, my name, role, company, mission, maintained preferences, locale, or location. Select this when the delivered authenticated profile supports the answer. This is profile/context lookup only; it is not stored-history retrieval, and a requested change is hivemind_profile_update.' },
  { id: 'hivemind_memory_lookup', criteria: 'Read stored HIVE-MIND memories, decisions, documents, evidence, past work, or historical facts. Select only when the user explicitly asks for saved, remembered, recorded, historical, past-work, or subject-specific information. Do not select for a generic identity/profile question such as what do you know about me when the authenticated profile is sufficient. The executor must form a concrete recall query. Canonical entity-hop-0 lookup already runs quickly and additively inside ordinary recall, so do not add a separate entity-search call as a preflight for a broad memory question.' },
  { id: 'hivemind_entity_lookup', criteria: 'Perform the fast, tenant-authorized canonical HIVE entity registry lookup when the user asks whether a named person, organization, project, product, document, or topic exists, asks who/what that entity is, or the name is ambiguous and resolving it is itself the next required step. Use the exact named subject and return canonical match/aliases or a healthy no-match. Do not use as an extra preflight for broad saved-history questions: hivemind_recall already runs bounded additive entity-hop-0 matching. A registry match alone is not evidence for biography, history, or relationships; follow with recall using authorized entity IDs only when the request asks for those details.' },
  ...
  { id: 'hivemind_save', criteria: 'Create one durable HIVE memory because the user explicitly asks to save, remember, retain, record, or file information already present in the request, recent turns, or completed governed receipts. Also select for a clear first-person or organization assertion that expresses durable context worth retaining without the literal word “save”, including recurring interests and durable likes, long-term preferences, personal working style, stated priorities, or reusable procedures. Treat a direct statement such as “I love to watch [sports/team/person]” as an implicit durable preference/interest, across languages; do not misclassify it as a request for scope clarification or ordinary small talk. Do not select for a transient greeting, question, hypothetical, passing reaction, or unsupported inference. Scope remains governed by the graph: ask only at the save checkpoint when required, never before a save decision. Do not select this when the same request first asks to retrieve, search, collect, read, verify, or pull information; that dependent read-plus-write request is multi_task.' },
