...
import { isProgressiveHarnessEnabled, resolveHarnessIntent, chooseProgressiveAction, buildProgressiveSynthesisMessages, boundedEvidence, parseProgressiveObject, buildProgressiveConversationContext, reviewProgressiveArguments, PROGRESSIVE_HARNESS_MODEL, PROGRESSIVE_HARNESS_PROVIDER } from './progressive-harness.js';
...
async function runProgressiveDurableAgent({ message, ctx, emit, composio, db, picked, run }) {
  ...
  const localize = async text => {
    try {
      ...
      const response = await chatCompletionFetch(PROGRESSIVE_HARNESS_MODEL, {
        ...
        body: JSON.stringify({ temperature: 0, max_tokens: 180, provider: PROGRESSIVE_HARNESS_PROVIDER, messages: [
  ...
  try {
    ...
    const synthesize = async () => {
      ...
      const { chatCompletionFetch } = await import('../llm/chat-provider.js');
      const response = await chatCompletionFetch(PROGRESSIVE_HARNESS_MODEL,
        { method: 'POST', signal: ctx._signal, body: JSON.stringify({ messages, temperature: 0.2, max_tokens: 1800, provider: PROGRESSIVE_HARNESS_PROVIDER }) }, { useCase: 'progressive_agent' });
    ...
    executionLoop: for (let index = 0; index < MAX_DURABLE_LOOP_STEPS; index += 1) {
      ...
      if (!hasParameters) generated = {};
        ...
        const { chatCompletionFetch } = await import('../llm/chat-provider.js');
        const response = await chatCompletionFetch(PROGRESSIVE_HARNESS_MODEL, { method: 'POST', signal: ctx._signal, body: JSON.stringify({ temperature: 0, max_tokens: 1800, provider: PROGRESSIVE_HARNESS_PROVIDER,
