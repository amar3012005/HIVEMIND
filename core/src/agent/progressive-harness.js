...
export async function chooseProgressiveAction({ observation, generateImpl, signal } = {}) {
  const raw = await decide(ACTION_SYSTEM, boundedEvidence(observation, PROGRESSIVE_PROMPT_BUDGETS.action), generateImpl, 'chat_planner', signal);
  ...
  return { action: raw.action, reason: clip(raw.reason, 700), ...(raw.slug ? { slug: clip(raw.slug, 150) } : {}),
    ...(raw.outcome_ids ? { outcome_ids: [...raw.outcome_ids] } : {}),
