...
export class CreditService {
  ...
  async reserve({ orgId, userId = null, service, units = 1, source = 'product', idempotencyKey, metadata = {} }) {
    ...
    return this.prisma.$transaction(async (tx) => {
      // pg_advisory_xact_lock returns PostgreSQL void. Prisma's queryRaw tries
      // to deserialize that value and fails before admission can complete;
      // executeRaw preserves the lock semantics without decoding a result row.
      await tx.$executeRawUnsafe(`SELECT pg_advisory_xact_lock(hashtext($1))`, `credits:${orgId}:${monthWindow().key}`);
