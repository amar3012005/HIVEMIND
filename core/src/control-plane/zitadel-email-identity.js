...
export function isZitadelEmailIdentityConfigured(env = process.env) {
  return Boolean(
    String(env.ZITADEL_API_URL || env.ZITADEL_ISSUER_URL || '').trim()
    && String(env.ZITADEL_SERVICE_PAT || '').trim()
    && String(env.ZITADEL_EMAIL_ORG_ID || '').trim(),
  );
}
...
export async function createZitadelEmailIdentity(email) {
  ...
  if (!isZitadelEmailIdentityConfigured()) throw new Error('ZITADEL email identity provisioning is not configured');
