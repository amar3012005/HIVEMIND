...
function discountLabel(discount) {
  if (!discount) return 'No payment today';
  if (discount.kind === 'percentage_discount') return `${discount.percent_off}% off after trial`;
  return `${(Number(discount.amount_off_cents) / 100).toFixed(2)} ${discount.currency || 'EUR'} off after trial`;
}
...
export function renderPartnerReferralInvitation({ referrerName, invitationUrl, offer, welcomeMessage = null }) {
  ...
  const body = `<tr><td class="section"><div class="eyebrow">A PRIVATE SINGULANCE INVITATION</div><h1 class="h1">${escapeHtml(name)} invited you to awaken your HIVEMIND.</h1><p class="copy">${escapeHtml(copy)}</p><table role="presentation" width="100%" class="stats"><tr><td class="stat"><div class="stat-label">FREE TRIAL</div><div class="stat-value">${trial} days</div></td><td class="stat"><div class="stat-label">MONTHLY CREDITS</div><div class="stat-value">${credits}</div></td><td class="stat"><div class="stat-label">INVITATION VALUE</div><div class="stat-value" style="font-size:15px">${escapeHtml(discountLabel(offer?.discount))}</div></td></tr></table><div class="founder-note"><div class="founder-quote">You are not entering an empty workspace. HIVEMIND will learn your company, prepare its first moves, and introduce the HyperAgents ready to work inside its memory.</div><div class="founder-signature">AMAR SAI · FOUNDER, SINGULANCE LABS</div></div><a class="action" href="${escapeHtml(invitationUrl)}">ACCEPT ${escapeHtml(name.toUpperCase())}'S INVITATION →</a><p class="copy" style="font-size:11px;color:${CARTESIA.muted}">This link may be shared by ${escapeHtml(name)}. The exact invitation terms are verified again when your workspace is created.</p></td></tr>`;
  return {
    subject,
    text: `${name} invited you to awaken your HIVEMIND. ${trial}-day free trial, ${credits} monthly credits. Accept: ${invitationUrl}`,
