...
// The explicit deployment flag supports local and self-hosted installations;
// otherwise Flagship and network failures continue to fail closed.
export async function entityDiscoveryCanaryFor({ orgId, userId, email, fetchImpl = fetch, logger = console }) {
  if (process.env.HIVEMIND_ENTITY_DISCOVERY_ENABLED === 'true') return true;
  if (process.env.HIVEMIND_ENTITY_DISCOVERY_ENABLED === 'false') return false;
  const config = configuration();
  const normalizedEmail = String(email || '').trim().toLowerCase();
  if (!config || !orgId || !userId || !normalizedEmail) return false;
  try {
