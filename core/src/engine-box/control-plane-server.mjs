...
<fieldset><legend>3. Customer-local models</legend><p class="hint">All three routes must remain on your network. Cloudflare AI Gateway is disabled by default.</p><label>Embedding endpoint<input required name="embedding_url" type="url"></label><label>Embedding model<input required name="embedding_model"></label><label>Embedding dimension<input required name="embedding_dimension" type="number" min="1" value="1024"></label><label>Rerank endpoint<input required name="rerank_url" type="url"></label><label>Rerank model<input required name="rerank_model"></label><label>Chat endpoint<input required name="chat_url" type="url"></label><label>Chat model<input required name="chat_model"></label></fieldset>
...
<script>const s=document.querySelector('#status'),f=document.querySelector('#setup');async function status(){const r=await fetch('/v1/setup/status');s.textContent=JSON.stringify(await r.json(),null,2)}status();f.onsubmit=async e=>{e.preventDefault();const d=Object.fromEntries(new FormData(f));const payload={access:{mode:d.access_mode,hostname:d.lan_hostname||null},oidc:{issuer:d.issuer,client_id:d.client_id,client_secret:d.client_secret,redirect_url:d.redirect_url,group_mapping:d.owner_group?{owner:[d.owner_group]}:{}},model_routes:{embedding:{execution:'local',base_url:d.embedding_url,model:d.embedding_model,dimension:Number(d.embedding_dimension)},rerank:{execution:'local',base_url:d.rerank_url,model:d.rerank_model},chat:{execution:'local',base_url:d.chat_url,model:d.chat_model}},backup:{destination:d.backup_destination,encryption_key_reference:d.backup_key_reference}};const r=await fetch('/v1/setup/configure',{method:'POST',headers:{'content-type':'application/json','x-engine-box-setup-token':d.setup_token},body:JSON.stringify(payload)});s.textContent=JSON.stringify(await r.json(),null,2);if(r.ok){f.querySelector('[name=client_secret]').value='';f.querySelector('[name=setup_token]').value='';s.className='status warn';s.textContent+='\n\nConfiguration is saved. The appliance will remain unavailable until its authenticated functional canary is implemented and passed.'}}</script>`;
}
