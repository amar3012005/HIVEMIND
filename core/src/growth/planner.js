...
export function normalizeGrowthPlanEvidence(plan, context) {
  ...
  if (Array.isArray(plan.operating_queue)) {
    plan.operating_queue = plan.operating_queue.map((item) => ({
      ...item,
      execution_brief: item?.execution_brief && typeof item.execution_brief === 'object'
        ? { ...item.execution_brief, evidence_refs: withBaseline(item.execution_brief.evidence_refs) }
        : item?.execution_brief,
    }));
  }
  return plan;
}
...
function validatePlan(plan, context, mode, aspects, lifecycleCatalog = []) {
  ...
  if (queue.some((item) => !item?.execution_brief || typeof item.execution_brief !== 'object'
    || !String(item.execution_brief.selected_approach || '').trim()
    || !String(item.execution_brief.why_now || '').trim()
    || !Array.isArray(item.execution_brief.evidence_refs)
    || !item.execution_brief.evidence_refs.includes(context.baseline.resource_id)
    || !Array.isArray(item.execution_brief.alternatives_considered))) {
    throw new Error('growth_plan_execution_brief_required');
  }
  plan.operating_queue = queue.map((item) => ({
    ...item,
    execution_brief: { ...item.execution_brief, decision_id: String(item.execution_brief.decision_id || item.id) },
