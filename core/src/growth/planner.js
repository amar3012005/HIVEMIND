...
export async function runGrowthPlan({ prisma, orgId, userId, mode = 'operate', aspects = [], objective = '', autonomyMode, turnId, hqCycleId, onProgress, model = 'gpt-oss-120b', lifecycleCatalog = [], additionalEvidence = null }) {
...
For initial_full return 2-4 ranked, genuinely evidenced queue items. SPAN THE MOTION FAMILIES the evidence supports instead of defaulting to strategy+campaign: when the plan needs positioning/audience clarity bind the marketing strategy lifecycle; when it needs real named prospects and personalized cold email bind the outreach prospect-to-conversation lifecycle (discover_and_qualify / prepare_personalized_messages); when specific warm prospects justify a voice conversation bind an outreach voice lifecycle (place_voice_call / operate_voice_call_cohort) — these are the TARA calls; and bind a campaign lifecycle for awareness. Competitor/market analysis has NO dedicated lifecycle: it is delivered inside the marketing strategy artifact's competitor_plan, so do not invent a competitor playbook and do not skip the strategy item when rivals are unknown. Order them so evidence-producing work precedes work that consumes it (strategy and prospect discovery before delivery or launch). Only include a family the retained evidence actually justifies — never pad the queue to cover every Room. Do not create busywork, prescribe a domain, pad the queue, or assign every Room. Every queue item must address an evidenced constraint, have an exact available room_tag, and be independently verifiable. The first queue item must match stage.queue_item_id and the primary constraint. Every factual assessment, constraint, and hypothesis must reference the baseline resource id or another supplied source reference.
