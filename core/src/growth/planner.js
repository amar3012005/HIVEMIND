...
For initial_full return 2-4 ranked, genuinely evidenced queue items. Do not create busywork, prescribe a domain, pad the queue, or assign every Room. Every queue item must address an evidenced constraint, have an exact available room_tag, and be independently verifiable. The first queue item must match stage.queue_item_id and the primary constraint. Every factual assessment, constraint, and hypothesis must reference the baseline resource id or another supplied source reference.
		external_action_requested describes the eventual requested terminal effect, not the authority of the current planning phase. Set it true when satisfying requested_terminal_outcome requires changing state outside Runtime's persisted internal artifacts; set it false only when internal evidence or preparation itself is the terminal outcome. All initial queue items still begin in PREPARE authority: they may research and persist internal deliverables, but may not make an external change. The selected playbook, not this planner, resolves capabilities and exact authority gates. Treat unavailable evidence as an explicit gap and do not block unrelated safe preparation.`;
