  ...
  async hydrateAdjacentEvidence({ anchors, userId, orgId, perDocument = 3, total = 12 }) {
    ...
    for (const item of matched) {
      ...
      windows.set(item.documentId, {
        gte: Math.max(0, item.metadata.segmentIndex - before),
        lte: item.metadata.segmentIndex + (perDocument - before - 1),
      });
    }
    const segments = await this.db.knowledgeSegment.findMany({
      where: {
        userId,
        orgId,
        OR: [...windows].map(([documentId, segmentIndex]) => ({ documentId, segmentIndex })),
      },
      include: { document: true },
      orderBy: [{ documentId: 'asc' }, { segmentIndex: 'asc' }],
      take: total,
    });
    const scores = new Map(matched.map((item) => [item.documentId, item.score ?? null]));
    return segments.map((segment) => ({
      ...
      content: segment.content,
      snippet: segment.content,
      score: scores.get(segment.documentId),
      document: segment.document,
      metadata: {
        segmentType: segment.segmentType,
        segmentIndex: segment.segmentIndex,
