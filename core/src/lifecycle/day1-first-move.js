...
export function renderDayTwoBrandDnaEmail({ companyName, output, roomUrl, characters = [], publicApiUrl = '', artifact = {} } = {}) {
  ...
  const snapshots = snapshotCells ? `<div style="margin-top:18px;font:700 8px/12px monospace;letter-spacing:1px;color:${CARTESIA.blue};text-transform:uppercase">Captured visual snapshots</div><table role="presentation" class="day2-snapshots" width="100%" cellspacing="0" cellpadding="0" style="margin-top:8px"><tr>${snapshotCells}</tr></table>` : '';  const card = (title, content) => `<td class="lifecycle-card" style="width:50%;vertical-align:top;border:1px solid #deddd8;padding:16px;background:#fff"><div style="font:700 8px/12px monospace;letter-spacing:1px;color:${CARTESIA.blue};text-transform:uppercase">${escapeHtml(title)}</div>${content}</td>`;
