...
function visualSnapshotUrl(key) {
  const base = String(process.env.HIVEMIND_VISUAL_ARTIFACT_URL || '').replace(/\/$/, '');
  const secret = String(process.env.HIVEMIND_VISUAL_WORKFLOW_SECRET || '');
  if (!base || !secret || !String(key).startsWith('org/')) return '';
  const expires = Date.now() + (30 * 24 * 60 * 60 * 1000);
  const sig = crypto.createHmac('sha256', secret).update(`${key}:${expires}`).digest('base64url');
  return `${base}/public-artifact?key=${encodeURIComponent(key)}&expires=${expires}&sig=${encodeURIComponent(sig)}`;
}
...
export function renderDayTwoBrandDnaEmail({ companyName, output, roomUrl, characters = [], publicApiUrl = '', artifact = {} } = {}) {
  ...
  const snapshotCells = evidence.slice(0, 3).map((item) => {
    ...
    return src ? `<td class="day2-snapshot" style="width:33.33%;padding:0 3px 0 0;vertical-align:top"><img src="${escapeHtml(src)}" alt="Captured website snapshot" width="180" style="display:block;width:100%;height:112px;object-fit:cover;border:1px solid #deddd8"></td>` : '';
  }).filter(Boolean).join('');
  const snapshots = snapshotCells ? `<div style="margin-top:18px;font:700 8px/12px monospace;letter-spacing:1px;color:${CARTESIA.blue};text-transform:uppercase">Captured visual snapshots</div><table role="presentation" class="day2-snapshots" width="100%" cellspacing="0" cellpadding="0" style="margin-top:8px"><tr>${snapshotCells}</tr></table>` : '';
  const customerLogo = visualSnapshotUrl(evidence.find((item) => item?.logo_r2_key)?.logo_r2_key);
  const customerBrand = customerLogo ? `<img src="${escapeHtml(customerLogo)}" alt="${escapeHtml(companyName)} logo" width="96" style="display:block;max-width:96px;max-height:58px;width:auto;height:auto;margin:18px 0 10px">` : '';
  ...
  const body = `<tr><td class="section" style="background:${CARTESIA.paper}"><div class="eyebrow">DAY 2 · YOUR AGENTS LEARNED YOUR BRAND</div>${customerBrand}<h1 class="h1">Your HyperAgents<br>mapped your visual language.</h1><p class="copy">Your source-backed Brand DNA is ready for ${escapeHtml(companyName)}.</p>${characterStrip(characters, { email: true, publicApiUrl })}${snapshots}${visualSystem}<div style="margin-top:18px;background:#101010;color:#fff;padding:18px"><div style="font:700 8px/12px monospace;letter-spacing:1px;color:#83b6ed;text-transform:uppercase">Reusable visual-artifact brief</div><div style="font:700 18px/23px Arial,sans-serif;margin-top:8px">${escapeHtml(brief.style || 'Evidence-first creative direction assembled from rendered first-party pages.')}</div>${briefItems ? `<ul style="margin:10px 0 0;padding-left:18px;font:12px/18px Arial,sans-serif;color:#e7e7e7">${briefItems}</ul>` : ''}</div><div style="margin-top:20px;font:700 8px/12px monospace;letter-spacing:1px;color:${CARTESIA.blue};text-transform:uppercase">Evidence ledger</div><h2 style="margin:6px 0 8px;font:700 20px/25px Arial,sans-serif">What the agents captured</h2><table role="presentation" width="100%" cellspacing="0" cellpadding="0">${evidenceRows || `<tr><td style="font:12px Arial,sans-serif">${escapeHtml(output)}</td></tr>`}</table><a class="action" href="${escapeHtml(roomUrl)}">OPEN THE FULL BRAND DNA REPORT →</a></td></tr>`;
