...
test('Flagship and Workflow receive only pseudonymous targeting and opaque job identity', async () => {
  await withWorkflowEnv(async () => {
    ...
    const enabledUrl = new URL(calls.at(-1).url);
    assert.match(enabledUrl.searchParams.get('targeting_key'), /^[a-f0-9]{64}$/);
    assert.equal(enabledUrl.searchParams.has('org_id'), false);
    assert.equal(enabledUrl.searchParams.has('user_id'), false);
    ...
    assert.deepEqual(admission, { job_id: JOB_ID, processing_version: 7, admitted: true });
...
test('Workflow status preserves structured state and distinguishes missing from outage', async () => {
  await withWorkflowEnv(async () => {
    ...
    const legacyNested = new CloudflareKnowledgeIngestClient({
      fetchImpl: async () => Response.json({ status: { status: 'waiting' } }),
    });
    assert.deepEqual(await legacyNested.getWorkflowStatus('wf-waiting'), { status: 'waiting' });
