...
test('GET /v1/hq/artifacts/:id returns a RuntimePlaybookArtifact, org-scoped', async () => {
  const prisma = {
    runtimePlaybookArtifact: {
      findFirst: async ({ where }) => {
        assert.equal(where.orgId, 'org-1');
        assert.deepEqual(where.OR, [{ id: 'artifact-1' }, { artifactId: 'artifact-1' }]);
        return { id: 'artifact-1', artifactKey: 'planning_evidence', data: { fact: 'value' }, createdAt: new Date('2026-08-18T00:00:00Z') };
      },
    },
  };
  const handle = handler(prisma);
  const res = fakeRes();
  const handled = await handle({ method: 'GET' }, res, fakeUrl('/v1/hq/artifacts/artifact-1'));
  assert.equal(handled, true);
  assert.equal(res.statusCode, 200);
  assert.equal(res.body.key, 'planning_evidence');
  assert.match(res.body.content, /"fact": "value"/);
});
