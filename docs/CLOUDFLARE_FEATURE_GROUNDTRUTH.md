...
The local runtime fails closed when its Flagship endpoint is unavailable or returns an invalid response. It must not use a local boolean environment variable as a fallback.
...
Only the entries above are migrated by this change. Do not infer a Flagship mapping for another `ENABLE_*`, `USE_*`, or tuning variable: many are infrastructure/model settings and need their own reviewed migration.
...
4. After this release is deployed and the Flagship endpoint is verified, remove the three `legacy_rollout_gate` variables listed by the script.
