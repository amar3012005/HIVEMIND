...
- Passed on `2026-07-14`: a source-specific scoped recall probe through the production identity contract returned `fact` in `1.131s` with 5 memories, `explain` in `1.027s` with 5 memories, and `full` in `644ms` with 5 memories plus 1 evidence section. None cut off. Browser-visible citations still require UI acceptance.
