...
## prod-20260716-5d661727 - Your Leads board + hard email dedup + HQ control-room feed

- **Parent:** `hivemind-main` at `5d6617272695c9a689b8453cd62bca9eba2d6a40` (post-merge SHA of PR #46 "feat(hq): control-room activity feed"; carries PR #44 leads/dedup + PR #42 kb quality + recall temporal indexes). All prior running tags verified ancestors of this SHA.
- **Frontend:** `frontend/Da-vinci` at `4af76871160ba9caf5ab850299f47c72c2daebb0` (`origin/fe-prospect-stack-on-prod`) — LeadsView + HQ control-room feed cards + dashboard right-rail counts.
- **Reconciliation:** at start `hm-core` ran `prod-20260716-45665f12`, `hm-control`/`hm-employees`/`tara`/`hivemind-next-frontend-1` ran `prod-20260716-c34a5469`, `hm-fe`(legacy 8088) `b9d6dc77`. All confirmed linear ancestors of `5d661727`. `hm-core-2` still absent (single replica). Rollback tags cut from actually-running images.
- **Scope:** `hm-control` (new `GET /v1/hyper/leads` + `GET /v1/hyper-rooms/:id/hq-activity` routes, hard cross-campaign email dedup in `outreach/campaigns.js`, `recordHqActivity` seal hook), `hm-core` (chat-recall-policy + document-first-ingestion changes from merged PRs; rebuilt for coherence), `hivemind-next-frontend-1` (LeadsView + HQ feed + right-rail counts). `hm-employees`/`tara-deepgram` UNCHANGED — retagged only (no source diff `c34a5469..5d661727`).
- **Build:** clean detached worktree `/root/builds/prod-20260716-5d661727` at `5d661727`, submodule `4af7687`, clean status asserted. Built via `scripts/release-singulance.sh 5d661727 core control-plane fe`.
- **Images:** core-api `sha256:8c8a321bc3dd…`; control-plane `sha256:42b6a8eb253b…`; frontend (next, `-single`) `sha256:cd2368b18283…`. employees/tara retagged from `c34a5469`.
- **Migrations:** two applied manually via `psql -v ON_ERROR_STOP=1` before promotion (both additive, `IF NOT EXISTS`, idempotent): `20260716120000_hq_activity` (NEW table `hivemind.hq_activity` + index; `down.sql` present) and `20260716120000_recall_temporal_indexes` (two partial indexes on `hivemind.memories`). `outreach_campaigns`/`outreach_targets` already present from `c34a5469` (not re-applied). Backup first: `/root/backups/hivemind/pre-5d661727-20260716T131825Z.dump` (21,036,676 bytes). Prod has no `_prisma_migrations` table (migrations always applied by raw SQL) — this is the source of the benign `P3005` at core boot (`prisma migrate deploy` bails on the non-empty un-baselined schema; app runs on the existing schema, `restarts=0`, healthy).
- **Env:** `VERSION`/`NEXT_VERSION` → `prod-20260716-5d661727`, both files backed up (`.bak-prod-20260716-5d661727`).
- **Recreate order (health-gated):** `control-plane` → `core` → `frontend` (next). All healthy/running. `hm-core-2` absent (not created); `hm-fe` legacy untouched.
- **Acceptance:** public `singulancelabs.com`/`next.../hivemind`/`api.../health`/`core.../health` all 200. New routes `GET /v1/hyper/leads` → 401 and `GET /v1/hyper-rooms/<zero-uuid>/hq-activity` → 401 (auth-gated, not 500/404). FE served chunks under `/srv/static/js/` contain `Your Leads`/`control room`/`hq-activity` markers (non-`main.js`). Zero fresh fatal/panic/OOM in `hm-control`/`hm-core`/frontend (only the benign boot `P3005` above). Not exercised: no real lead campaign sent this release; HQ feed verified via route auth + FE marker, not a live room-run report card.
- **Aliases:** `core-api`/`control-plane` `:stable`/`:latest` and `fe:stable-single`/`:latest-single` advanced after acceptance.
- **Rollback:** `hivemind/core-api:rollback-20260716-132043` (pre `45665f12`), `hivemind/control-plane:rollback-20260716-132043` (pre `c34a5469`), `hivemind/fe:rollback-20260716-132043-single` (pre `c34a5469-single`). Restore: retag back, restore `.bak-prod-20260716-5d661727` env files, recreate affected `--no-deps --force-recreate`. Migrations additive; `hq_activity` `down.sql` available (code-only rollback needs nothing — old code never queries the new table/indexes).
