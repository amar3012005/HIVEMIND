...
| 2026-06-06T18:52Z | GREEN ✅ | none | **P2 RECALL: measured live Recall@K. 40%→93.3%** after fixing eval probe-pool (was sampling non-recallable internal-audit/tara rows → fake misses). Real recall HEALTHY at 93% (14/15), p95 1820ms. Evolution gate metric now truthful (was about to optimize vs a lie). memory-eval.mjs deployed to prod (script-only, no restart). Recall engine untouched — no tuning needed. |
