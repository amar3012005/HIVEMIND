...
async def web_search_emulated(query: str, *, user_id: Optional[str], org_id: Optional[str],
    ...
    """Live web search through the tenant-scoped evidence boundary.

    The control plane attempts Composio Search first and governed Parallel Search
    second.  This client deliberately does *not* fall through to Core's legacy
    Tavily job API: doing so hides which provider supplied an evidence claim and
    turns a governed-provider outage into an untraceable semantic change.
    """
    settings = get_settings()
    headers = _emulated_headers(api_key, user_id, org_id)
    ...
    if user_id and org_id:
        ...
        try:
            ...
            try:
                ...
                pass
            log.warning("[web.search] governed evidence lane unavailable (%s)", response.status_code)
            return {
                "error": str(detail.get("error") or f"governed web search {response.status_code}")[:200],
                "provider_attempts": detail.get("provider_attempts") or [],
            }
        except Exception as exc:  # noqa: BLE001
            log.warning("[web.search] governed evidence lane failed (%s)", str(exc)[:160])
            return {"error": "governed web search unavailable", "provider_attempts": []}
    return {"error": "user and organization scope are required for governed web search", "provider_attempts": []}
