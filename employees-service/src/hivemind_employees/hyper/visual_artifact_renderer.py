...
def render_presentation(spec: dict[str, Any]) -> str:
    ...
    palettes = {
        "editorial": ("#151712", "#f2f0e8", "#ffffff", "#6d7067", "#d5d2c7"),
    ...
    return f'''<!doctype html><html lang="en" data-mode="{_esc(spec["visual_mode"])}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>{_esc(spec["title"])}</title><style>
:root{{--ink:{ink};--paper:{paper};--surface:{surface};--muted:{muted};--line:{line};--accent:{accent};--shadow:0 24px 80px #0008}}*{{box-sizing:border-box}}html,body{{margin:0;min-height:100%;background:#080908;color:var(--ink);font-family:Arial,Helvetica,sans-serif;letter-spacing:0}}body{{overflow:hidden}}.deck{{height:100vh;display:grid;place-items:center;padding:18px}}.slide{{display:none;width:min(151vh,calc(100vw - 36px));height:min(84.9vw,calc(100vh - 36px));aspect-ratio:16/9;background:var(--paper);padding:clamp(28px,4vw,68px);position:relative;overflow:hidden;grid-template-columns:repeat(12,minmax(0,1fr));grid-template-rows:auto minmax(0,1fr) auto;gap:clamp(18px,2vw,34px);box-shadow:var(--shadow)}}.slide.active{{display:grid}}.slide:before{{content:attr(data-slide);position:absolute;right:-.02em;bottom:-.24em;font-size:clamp(220px,28vw,440px);line-height:1;font-weight:900;color:color-mix(in srgb,var(--ink) 4%,transparent);pointer-events:none}}.slide>header{{grid-column:1/-1;display:flex;justify-content:space-between;align-items:center;text-transform:uppercase;font-size:10px;font-weight:800;letter-spacing:1.6px;color:var(--muted);border-bottom:1px solid var(--line);padding-bottom:12px;z-index:2}}.slide>header span{{color:var(--accent)}}.slide-copy{{position:relative;z-index:2;align-self:center}}h1,h2{{font-size:clamp(42px,5.5vw,86px);line-height:.94;margin:0 0 22px;letter-spacing:0;font-weight:850;text-wrap:balance}}.slide-copy>p{{font-size:clamp(15px,1.28vw,20px);line-height:1.48;color:var(--muted);max-width:620px;margin:0}}.slide>footer{{grid-column:1/-1;display:flex;align-items:center;gap:16px;border-top:1px solid var(--line);padding-top:11px;color:var(--muted);font-size:9px;text-transform:uppercase;letter-spacing:.7px;white-space:nowrap;overflow:hidden;z-index:2}}.slide>footer span{{overflow:hidden;text-overflow:ellipsis}}.slide>footer i{{height:2px;background:var(--accent);flex:1;min-width:40px}}.visual{{align-self:center;margin:0;display:grid;position:relative;z-index:2;min-width:0}}.visual article{{position:relative;min-width:0}}.item-index{{font-size:10px;color:var(--accent);font-weight:900}}.visual article small{{display:block;text-transform:uppercase;font-size:9px;letter-spacing:1.15px;color:var(--muted)}}.visual article strong{{display:block;font-size:clamp(22px,2.65vw,42px);line-height:1;margin:8px 0 10px}}.visual article p{{font-size:12px;line-height:1.42;color:var(--muted);margin:0}}.visual article em{{font-style:normal;text-transform:uppercase;font-size:8px;letter-spacing:.8px;color:var(--muted)}}
.composition-hero{{background:var(--ink);color:var(--paper)}}.composition-hero>header,.composition-hero>footer{{border-color:color-mix(in srgb,var(--paper) 18%,transparent);color:color-mix(in srgb,var(--paper) 65%,transparent)}}.composition-hero .slide-copy{{grid-column:1/9;grid-row:2;align-self:end}}.composition-hero h1{{font-size:clamp(58px,7.4vw,116px);max-width:1050px}}.composition-hero .slide-copy>p{{color:color-mix(in srgb,var(--paper) 70%,transparent);max-width:720px}}.composition-hero .visual{{grid-column:1/-1;grid-row:3;display:flex;border-top:1px solid color-mix(in srgb,var(--paper) 18%,transparent);transform:translateY(-34px)}}.composition-hero .visual article{{flex:1;padding:18px 22px 0 0;color:var(--paper)}}.composition-hero .visual article+article{{border-left:1px solid color-mix(in srgb,var(--paper) 18%,transparent);padding-left:22px}}.composition-hero .visual article p,.composition-hero .visual article em{{color:color-mix(in srgb,var(--paper) 60%,transparent)}}.composition-hero>footer{{display:none}}
...
.composition-decision{{background:var(--accent);color:#111}}.composition-decision .slide-copy{{grid-column:1/9;grid-row:2}}.composition-decision h2{{font-size:clamp(58px,7vw,108px)}}.composition-decision .slide-copy>p{{color:#253019;max-width:760px}}.composition-decision .visual{{grid-column:9/-1;grid-row:2;align-self:end;border-top:2px solid #111}}.visual-decision article{{padding:15px 0;border-bottom:1px solid #0003}}.composition-decision .visual article strong{{font-size:22px}}.composition-decision .visual article p,.composition-decision .visual article em,.composition-decision .visual article small{{color:#26321d}}
html[data-mode="cinematic"] .slide,html[data-mode="data-room"] .slide,html[data-mode="technical"] .slide{{color:var(--ink)}}html[data-mode="cinematic"] h1,html[data-mode="cinematic"] h2{{font-weight:700}}html[data-mode="editorial"] h1,html[data-mode="editorial"] h2,html[data-mode="minimal-luxury"] h1,html[data-mode="minimal-luxury"] h2{{font-family:Georgia,"Times New Roman",serif;font-weight:700}}html[data-mode="technical"] .slide{{background-image:linear-gradient(#ffffff06 1px,transparent 1px),linear-gradient(90deg,#ffffff06 1px,transparent 1px);background-size:36px 36px}}html[data-mode="minimal-luxury"] .slide>header span{{font-family:Georgia,"Times New Roman",serif;font-style:italic;text-transform:none;font-size:12px}}.controls{{position:fixed;right:28px;bottom:24px;display:flex;align-items:center;gap:8px;z-index:20}}.controls button{{width:44px;height:44px;border:1px solid #ffffff44;background:#111;color:#fff;cursor:pointer;font-size:19px}}.controls button:hover,.controls button:focus-visible{{background:var(--accent);color:#111;outline:2px solid #fff;outline-offset:2px}}.position{{color:#fff;font-size:10px;text-transform:uppercase;margin-right:8px;letter-spacing:1px}}
@media(max-width:680px){{body{{overflow:auto;background:var(--paper)}}.deck{{height:auto;display:block;padding:0}}.slide,.slide.active{{display:grid;width:100%;height:auto;min-height:100svh;aspect-ratio:auto;padding:25px 20px;grid-template-columns:1fr;grid-template-rows:auto auto 1fr auto;gap:26px;box-shadow:none}}.slide:before{{font-size:180px;bottom:-.1em}}.slide>header{{grid-column:1;grid-row:1}}.slide-copy,.composition-hero .slide-copy,.composition-thesis .slide-copy,.composition-comparison .slide-copy,.composition-process .slide-copy,.composition-matrix .slide-copy,.composition-timeline .slide-copy,.composition-decision .slide-copy{{grid-column:1;grid-row:2;align-self:end;padding:0}}h1,h2,.composition-hero h1,.composition-thesis h2,.composition-decision h2{{font-size:clamp(40px,12vw,58px)}}.slide-copy>p{{font-size:15px}}.visual,.composition-hero .visual,.composition-thesis .visual,.composition-comparison .visual,.composition-process .visual,.composition-matrix .visual,.composition-timeline .visual,.composition-decision .visual{{grid-column:1;grid-row:3;align-self:start;width:100%;display:grid;grid-template-columns:1fr;transform:none;border-top:1px solid var(--line);background:transparent;gap:0;padding:0}}.visual article,.composition-hero .visual article,.visual-comparison article{{min-height:0;padding:14px 0;border:0;border-bottom:1px solid var(--line);transform:none;background:transparent}}.visual-process:before{{display:none}}.visual-process .item-index{{position:static;width:auto;height:auto;display:inline;background:transparent}}.visual-timeline article:before{{display:none}}.slide>footer{{grid-column:1;grid-row:4;white-space:normal}}.composition-hero>footer{{display:flex}}.composition-hero .visual article,.composition-hero .visual article p,.composition-hero .visual article em{{color:var(--paper)}}.controls{{display:none}}}}
