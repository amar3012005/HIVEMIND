...
import { evaluateEntityDiscoveryCanary, evaluateGovernedRoomCanary, evaluateHyperPlannerMode, evaluateOperatingRoomCanary, evaluateProjectionMode, evaluateRecallReliability, evaluateTaraGrokCanary } from './flags';
...
export default {
  async fetch(request: Request, env: RuntimeEnv): Promise<Response> {
    ...
    if (url.pathname === '/tara-grok-enabled' && request.method === 'GET') {
      const orgId = url.searchParams.get('org_id') || '';
      const userId = url.searchParams.get('user_id') || '';
      const email = url.searchParams.get('email') || '';
      return Response.json({ enabled: await evaluateTaraGrokCanary(env, orgId, userId, email) });
    }
