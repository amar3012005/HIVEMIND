...
export async function evaluateHarnessChatMode(env: Env, orgId: string, userId: string): Promise<{
  ...
  if (env.ENVIRONMENT === 'local') return { ...fallback, variation: 'harness' };
  try {
...
function authCallback(request: Request): Response {
  ...
  const hiveOverview = `${origin}/hivemind/app/overview/new`;
...
async function staticHarness(request: Request, env: Env): Promise<Response> {
  ...
  if (!(asset.headers.get('content-type') || '').includes('text/html')) return assetSecurityHeaders(asset, env);
  const boot = await proxyRunner(new Request(new URL('/api/hivemind/boot', request.url), { headers: request.headers }), env);
  if (!boot.ok) {
    const parent = String(env.HIVE_HARNESS_PARENT_ORIGINS || '').split(',').map(value => value.trim()).find(Boolean);
    return new Response(null, { status: 302, headers: {
      'cache-control': 'no-store',
      location: new URL('/hivemind/login', parent || request.url).toString(),
  ...
  const supportedEnvelope = payload.version === 1 || (payload.version === undefined && payload.ok === true);
  if (!supportedEnvelope || !Array.isArray(payload.injections)) return new Response('HIVE-MIND boot unavailable', { status: 503 });
  const headers = new Headers(asset.headers);
  headers.delete('content-length');
  headers.set('cache-control', 'private, no-store');
  return assetSecurityHeaders(new Response(renderBootIndex(await asset.text(), payload.injections), { status: asset.status, headers }), env);
}
