...
type ContextReceipt = { job: any; request: any; company_context: any; brand_dna: any };
  ...
  try {
    const result = await (env.AI as any).run(model, {
      messages: [
        ...
        { role: 'user', content: `Act as a strict senior visual-design critic. Score 0-100 for prompt adherence, brand fit, composition, material/lighting quality, absence of text/logo artifacts, and production readiness. Return JSON only: {score,needs_revision,revision_instruction,findings}. Contract: ${JSON.stringify(spec).slice(0, 12_000)}` },
      ],
      image: dataUri,
      max_tokens: 900,
      temperature: 0,
    }, env.AI_GATEWAY_ID ? { gateway: { id: env.AI_GATEWAY_ID } } : undefined);
    const parsed = parseJson(result); return { model, score: Math.max(0, Math.min(100, Number(parsed.score) || 0)), needs_revision: parsed.needs_revision === true || Number(parsed.score) < 75, revision_instruction: String(parsed.revision_instruction || '').slice(0, 1200), findings: parsed.findings || [] };
    ...
    try {
      ...
      if (count > 1) {
        ...
        anchor = await step.do('generate-style-anchor', { retries: { limit: 6, delay: '20 seconds', backoff: 'exponential' }, timeout: '20 minutes' }, async () => {
          const generated = await generateAnchor(this.env, finalPrompt(direction.production_spec, direction.production_spec.variants[0], false), quality);
      ...
      await step.do('event-master', () => report(this.env, trigger, 'generating_master', 'generating_master', 58, 'Master visual generated.', { model: master.model }));
