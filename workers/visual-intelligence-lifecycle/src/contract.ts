...
export type Trigger = { job_id: string; org_id: string; user_id: string; urls: string[]; mode: Mode; deliverable: Deliverable; processing_version: number; requested_at: string; room_id?: string; lifecycle_day?: 2 };
...
export function validTrigger(value: unknown): value is Trigger {
  ...
  return !!p && UUID.test(String(p.job_id)) && UUID.test(String(p.org_id)) && UUID.test(String(p.user_id))
    ...
    && MODES.includes(p.mode) && DELIVERABLES.includes(p.deliverable) && Number.isInteger(p.processing_version) && p.processing_version > 0
